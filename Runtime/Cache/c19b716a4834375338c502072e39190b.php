    ...
    <div class="container">
      ...
      <form id="infoForm" action="__APP__/Profile/changeProfile" method="post" enctype="multipart/form-data">
        ...
        <div class="info-block">
          ...
          <input type="text" id="publicName" name="publicName" class="info-value" maxlength="15" value="<?php echo ($user["name"]); ?>"><span></span>
        </div>
        <div class="info-block">
          <div class="info">邮  箱：</div>
          <input type="text"  name="email" class="info-value" id="email" placeholder = "@" value="<?php echo ($user["email"]); ?>" >
          <span class="tip">请填写有效的电子邮箱地址，以便于找回密码</span>
        </div>
        <div class="info-block">
          <div class="info">网站/微博：</div>
          <input type="text" name="websiteORweibo" class="info-value" placeholder="http://" value = "<?php echo ($user["weibo"]); ?>">
        </div>
        <div class="info-block">
          <div class="info">微  信：</div>
          <input type="text" name="weixin" class="info-value" placeholder="微信号/QQ/手机" value = "<?php echo ($user["wechat"]); ?>">
        </div>
        <div class="info-block">
          <div class="info">社团简介：</div>
          <textarea cols="100" name="intro" rows="5" id="intro" class="info-value" placeholder="不要超过2000字哦" maxlength="2000"><?php echo ($user["introduction"]); ?></textarea>
        </div>
